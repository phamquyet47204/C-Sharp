...
/// </summary>
public class GeofenceService(LocalDatabase db, NarrationEngine narration, AnalyticsService analytics)
{
    ...
    private async Task CheckGeofencesAsync(double lat, double lon)
    {
        ...
        foreach (var poi in pois) // already sorted by Priority desc
        {
            ...
            if (dist <= poi.Radius)
            {
                // Track lượt ghé thăm POI
                _ = analytics.TrackPoiVisitAsync(poi.BasePoiId, lat, lon);
