...
/// <summary>Converter dùng trong XAML để đảo bool.</summary>
...
public partial class MapPage : ContentPage
{
    ...
    public MapPage(MapViewModel vm, NarrationEngine narration)
    {
        ...
        _vm.NavigateToPoiOnMap += OnNavigateToPoiOnMap;
        _vm.ListenRequested += async poi => await _narration.EnqueueAsync(poi);
    ...
    // Tap vào bản đồ → tìm pin gần nhất trong ~60m, hiện POI card
    private void OnMapClicked(object? sender, MapClickedEventArgs e)
    {
        const double thresholdDeg = 0.0006; // ~60m
    ...
    private void ShowPoiCard(PoiRecord poi)
    {
        ...
        PoiCategoryLabel.Text = $"🍜 {poi.CategoryDisplayName}";
    ...
    {
        PremiumSheet.IsVisible = false;
        await DisplayAlert("Mua Access Pass",
    ...
    private static ImageSource CreatePoiImageSource(string imagePath)
    {
        if (Uri.TryCreate(imagePath, UriKind.Absolute, out var absoluteUri))
        {
            ...
            {
                absoluteUri = new UriBuilder(absoluteUri) { Host = "10.0.2.2" }.Uri;
            }
